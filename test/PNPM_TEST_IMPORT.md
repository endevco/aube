...
- [ ] `pnpm/test/install/lifecycleScripts.ts` (21 tests, 356 LOC) → folded into [test/lifecycle_scripts.bats](lifecycle_scripts.bats) (12/21 ported, [#421](https://github.com/endevco/aube/pull/421))
  ...
  - Aube divergences worth noting in ports: aube writes the literal boolean `false` for review placeholders where pnpm writes the string `"set this to true or false"`; aube's strict-dep-builds error reads "dependencies with build scripts must be reviewed before install" where pnpm's reads "Ignored build scripts:".
  ...
  - Documented divergences (don't port without aube-side fix): `--allow-build=<pkg>` CLI flag (149, 164, 347 — aube has `aube approve-builds` interactive flow but no per-add CLI surface), `verify-deps-before-run` interaction with preinstall sub-aube calls (179, 200 — aube's auto-install gate has different recursion semantics), selective `aube rebuild <pkg>` (282 — aube's `aube rebuild` rebuilds the whole policy, no per-package selector).
