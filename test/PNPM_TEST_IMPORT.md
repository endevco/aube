...
- [ ] `pnpm/test/install/misc.ts` (37 tests, 645 LOC) → [test/pnpm_install_misc.bats](pnpm_install_misc.bats) (8/37 ported)
  ...
  - Documented divergences (don't port without aube-side fix): create-package.json-if-missing (233 — aube errors), `pnpm add` with no name exits 1 (245 — aube exits 0), `package.yaml` manifest (136 — aube doesn't support), `--lockfile-dir` (112 — aube has no flag for placing the lockfile outside the project root).
