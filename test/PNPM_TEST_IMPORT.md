...
- [ ] `pnpm/test/install/misc.ts` (37 tests, 645 LOC) → [test/pnpm_install_misc.bats](pnpm_install_misc.bats) (11/37 ported)
  ...
  - Documented divergences (don't port without aube-side fix): `package.yaml` manifest (136 — aube's manifest reader is `package.json`-only; would need a multi-crate format-aware rewrite).
