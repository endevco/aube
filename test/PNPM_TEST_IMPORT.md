...
- [ ] `pnpm/test/update.ts` (22 tests, 50 dist-tag uses) → [test/pnpm_update.bats](pnpm_update.bats) (7/22 ported)
  - Done: update <dep> (14), update --no-save (34), update --latest --prod (225), recursive update --no-save (72), recursive update --no-shared-workspace-lockfile (118), recursive update --latest on shared lockfile (426 — asserted per-project, see divergence below), recursive update --latest --prod on shared lockfile (478 — same).
  - Skipped (still need fixtures): update --latest (143), update --latest --save-exact (170), update --latest specific dep (197), recursive update --latest specific no-shared (369), recursive update --latest specific shared (543) — all need `@pnpm.e2e/qar` (alias dep) + GitHub spec support (`kevva/is-negative`). update to latest without downgrading prerelease (615), update with tag @latest downgrades prerelease (659), update to latest recursive prerelease workspace (728, 807) — all need `@pnpm.e2e/has-prerelease`. recursive update --latest no-shared (249) and recursive update --latest --prod no-shared (302) are subsumed by the shared-lockfile ports above (aube's `update -r` always writes per-project lockfiles regardless of `sharedWorkspaceLockfile`).
