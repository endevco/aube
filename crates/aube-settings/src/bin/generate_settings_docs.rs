...
fn render_page(settings: &[SettingRef<'_>]) -> String {
    ...
    writeln!(out, "## Browse by category").unwrap();
    writeln!(out).unwrap();
    ...
    writeln!(out, "## Contributing to this reference").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "This page is generated from [`settings.toml`](https://github.com/aubepkg/aube/blob/main/crates/aube-settings/settings.toml). Edit the registry and run `{COMMAND}` to regenerate it.").unwrap();
...
fn render_summary(out: &mut String, settings: &[SettingRef<'_>]) {
    ...
    writeln!(out, "<summary>Expand the complete settings index</summary>").unwrap();
    writeln!(out).unwrap();
