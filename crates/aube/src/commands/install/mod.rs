...
pub async fn run(opts: InstallOptions) -> miette::Result<()> {
    ...
    // *and* every speculative transitive name reachable through the
    // bundled primer's dep graph (those that aren't primer-covered).
    // Runs before lockfile parse, pnpmfile setup, and resolver
    // construction — by the time the resolver pops its first task,
    // the packument is in the on-disk cache and the reqwest pool is
    // hot, and the BFS-depth RTT chain (real-npmjs cold-install
    // bottleneck) is collapsed into one parallel fan-out.
    //
    // `needs_time` controls cache selection (full vs corgi packument)
    // so the prefetched bytes land in the cache the resolver actually
    // reads next. The resolver makes the same determination from the
    // same settings later; mirroring it here keeps the prefetch
    // effective for the default-aube case (trustPolicy=no-downgrade
    // + minimumReleaseAge=1440 → needs_time=true → full packument).
    // Honors `AUBE_DISABLE_PREFETCH=1`.
    let prefetch_needs_time = needs_time_for_prefetch(&settings_ctx);
    prefetch::spawn_packument_prefetch(&manifest, &cwd, opts.network_mode, prefetch_needs_time);
