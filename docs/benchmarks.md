...
The chart highlights the warm-cache, no-`node_modules` case: aube's
advantage there comes from reusing a shared global virtual store, so
projects mostly link to package trees that already exist.
