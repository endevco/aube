...
- **aube-store** — Content-addressable store under `$XDG_DATA_HOME/aube-store/v1/files/` (defaulting to `~/.local/share/aube-store/v1/files/`; legacy `~/.aube-store/` is reused when present) using BLAKE3 with 2-char directory sharding. Tarball integrity is still SHA-512 because that's the registry format
...
- **Aube-owned global store**: `$XDG_DATA_HOME/aube-store/v1/files/` (defaulting to `~/.local/share/aube-store/v1/files/`; legacy `~/.aube-store/` is reused when present) is a BLAKE3 CAS with 2-char directory sharding. Aube never reads from or writes to `~/.pnpm-store/`.
